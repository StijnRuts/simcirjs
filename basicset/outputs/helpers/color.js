...
const _multiplyColor = (iColor1, iColor2, ratio) => {
  const c1 = toRGB(iColor1);
  const c2 = toRGB(iColor2);
  const mc = (v1, v2, ratio) =>
    ~~Math.max(0, Math.min((v1 - v2) * ratio + v2, 255));
...
export const multiplyColor = (color1, color2, ratio) => {
